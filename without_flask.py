...
import missingno as msno # مكتبة متخصصة في تصوير القيم المفقودة

import plotly.express as px #لعمل رسومات Interactive بسرعة بسطر واحد.

import plotly.figure_factory as ff # Figure Factory لرسومات متخصصة مش متاحة مباشرة في px >> زي توزيعات و الكثافه مثلا
import plotly.graph_objects as go

from wordcloud import WordCloud # نستخدمها لإعطاء لمحة سريعة عن الكلمات الأكثر شيوعًا في نصوص
...
- **person_age** >>> عمر المتقدِّم بالسنوات وقت التقديم  
- **person_income** >>> الدخل السنوي الإجمالي للمتقدِّم  
- **person_home_ownership** >>> حالة سكن المتقدِّم (Own/Rent/Mortgage/Other)  
- **person_emp_length** >>> مدة الخبرة العملية بالسنوات  
- **loan_intent** >>> الغرض من القرض (مثلًا: تعليم، سيارة، أعمال...)  
- **loan_grade** >>> درجة/تصنيف جودة القرض حسب الجهة المموِّلة (A–G عادةً)  
- **loan_amnt** >>> قيمة القرض المطلوبة أو المصروفة  
- **loan_int_rate** >>> معدل الفائدة السنوي للقرض (نسبة مئوية)  
- **loan_status** >>> حالة الهدف (سداد ناجح أم تعثّر عن السداد)  
- **loan_percent_income** >>> نسبة قيمة القرض إلى دخل المتقدِّم (loan_amnt / income)  
- **cb_person_default_on_file** >>> وجود تعثّر سابق في السجل الائتماني (Y/N)  
- **cb_person_cred_hist_length** >>> طول السجل الائتماني للمتقدِّم (بالسنوات)
**bold text**
...
#  الارتباط فقط بين الأعمدة الرقمية
correlation_table = data.select_dtypes(include=[np.number]).corr()
# اختار الأعمدة اللي نوعها رقمي فقط

print("Correlation matrix:\n")
print(correlation_table)

# person_age ↔ cb_person_cred_hist_length → 0.86   علاقة طردية قوية جدًا
# loan_amnt ↔ loan_percent_income → 0.57        علاقة طردية متوسطة
# loan_status ↔ loan_int_rate → 0.34              علاقة طردية متوسطة (الفائدة العالية اقرب للتعثر)
# loan_status ↔ loan_percent_income → 0.38         علاقة متوسطة (نسبة القرض للدخل تؤثر على السداد)
# person_income ↔ loan_amnt → 0.27                 علاقة ضعيفة

"""## Univarient Analysis"""

#MAX AND MIN AGE
#تحليل عمود person_age لمعرفة:
# أكبر وأصغر عمر موجود في الداتا.
...
print("_"*70)
# توزيع الناس على فئات عمرية (من 0–18، 19–26، 27–36 وهكذا).
...
print(f"minimum Income {min_}")
 ...
 # np.inf >>> "Infinity" = عدد لا نهائي (∞) >>
 # كحدّ أعلى مفتوح للفئة الأخيرة
# عدّ أصحاب الدخل صفر + (اختياري) المفقودات
...
# RENT → ساكن إيجار
# OWN → عنده بيت ملك
# MORTGAGE → شاري بيت بقرض (تمويل عقاري)
# OTHER → نوع سكن آخر
...
#  أعمدة توضح توزيع أغراض القروض كنِسَب.
...
# لتسهيل القراءة :
# كوّن عمود جديد اسمه age_group
# عن طريق تقسيم عمر الشخص (person_age) إلى فئات عمرية محددة
...
# لتسهيل القراءة ايضا
# كوّن عمود جديد اسمه income_group
# عن طريق تقسيم مرتب الشخص (person_income) إلى فئات مالية محددة
...
# كون عمود جديد لتصنيف القروض

data['loan_amount_group']

data.head()

# خلق أعمدة جديدة تمثل نِسَب بين عمودين علشان >> تطبيع المقاييس بدل ما تبص للمبلغ والدخل منفصلين >> التقاط علاقات غير خطية
# ربط بين مقادير مهمة بدل ما تبص لكل عمود لوحده

# نسبة القرض إلى الدخل. كل ما تكبر النسبة، العبء النسبي للقرض أعلى >> غالبًا مخاطرة أعلى بالتعثّر.
# Create loan-to-income ratio
data['loan_to_income_ratio'] = data['loan_amnt'] / data['person_income']

# طول الخبرة لكل وحدة من مبلغ القرض. خبرة أكبر مقابل قرض أصغر تعطي قيمة أعلى (قد تعني استقرار نسبي)
# Create loan-to-employment length ratio
data['loan_to_emp_length_ratio'] =  data['person_emp_length']/ data['loan_amnt']

# سعر الفائدة لكل وحدة من مبلغ القرض
...
drop_colums = [] # قائمة الأعمدة المراد حذفها ، بنحط فيها أي أعمدة هتستبعدها
...
# الأعمدة العددية اللي هتتعملّها >>> Scaling (Standard/MinMa) >>> زي person_income, loan_amnt
...
#الأعمدة اللي هتتعملّها >> One-Hot Encoding >> زي loan_grade, person_home_ownership >> وكمان الاعمدة الجديده زي income_group, age_group, loan_amount_group

le_colums = [] # احتياطي لو احتجت اعمل Label Encoding
# حاليا مفيش
...
X = data.drop(['loan_status'], axis=1) # axis=1 يعني على الأعمدة
Y = data['loan_status']
...
#  تجميع كل الفئات الأصلية التي تعلّمها الـ
# OneHotEncoder
# من أول 7 أعمدة فئوية في مصفوفة واحدة.

merge_ohe_col

# تجميع الوان هوت انكودر اعمدة التدريب
ohe_data = pd.DataFrame(ohe.transform(x_train[ohe_colums]).toarray(), columns=merge_ohe_col)
# ohe.transform >> يحوّلها لمصفوفة One-Hot (0/1) بناءً على الفئات التي تعلّمها في fit
# toarray() >> حوّل الناتج من مصفوفة sparse matrix متناثره او مش متجمعه >>>
# الي  pd.DataFrame

# تجميع الوان هوت انكودر اعمدة التيست
...
# ضم الأعمدة الـ One-Hot الجديده
# مع الداتا الأصلية

X_new_test = pd.concat([ohe_data2, x_test], axis=1) # نفس الكلام للتيست
...
# اختيار السكّيلر المناسب لكل نمط توزيع.

from sklearn.preprocessing import MinMaxScaler,StandardScaler,RobustScaler
# مناسب لو مفيش آوتلايرز قوية >> MinMaxScaler >>> يعيد قياس القيم إلى مجال ثابت (افتراضيًا [0, 1]).
# StandardScaler >> يركز البيانات حول المتوسط ويجعل الانحراف المعياري ≈ 1
# RobustScaler >> يعتمد على الميديان و IQR >> فا بيتعامل احسن مع الاوتلايرز
...
scale_cols = ['person_income','person_age','person_emp_length', 'loan_amnt','loan_int_rate','cb_person_cred_hist_length','loan_percent_income', 'loan_to_emp_length_ratio',
       'int_rate_to_loan_amt_ratio']
       # قائمة عامة بالأعمدة المراد عمل سكيلينج لها
...
bimodal_col = [] # ليست للأعمدة اللي توزيعها فيه آوتلايرز قوية هتستخدمها بعدين
...
from sklearn.svm import SVC  # يفصل الحدود بأكبر هامش و كويس اوي على الداتا اللي ليها فيتشرز صغيرة/متوسطة

from sklearn.naive_bayes import MultinomialNB  # مصنّف بايز مناسب لبيانات عدّ/Counts غير سالبة (نصوص/TF)

from sklearn.tree import DecisionTreeClassifier  # شجرة قرار للتصنيف؛ تلتقط اللاخطّية والتفاعلات ببساطة

from sklearn.neighbors import KNeighborsClassifier  # مصنّف أقرب الجيران؛ يحتاج Scaling وغالي حسابيًا وقت التست

from sklearn.ensemble import RandomForestClassifier  # غابة عشوائية قوية وثابتة؛ تقليل تباين + أهمية خصائص

from sklearn.ensemble import AdaBoostClassifier  # مفيد على داتا نظيفة وحدود واضحة (Boosting بمتعلمين ضعفاء)

from sklearn.ensemble import BaggingClassifier  # مجمّع Bagging للتصنيف يقلّل التباين عبر Bootstrap

from sklearn.ensemble import ExtraTreesClassifier  # أشجار شديدة العشوائية؛ سريعة وأحيانًا أقل تباين

from sklearn.ensemble import GradientBoostingClassifier  # Gradient Boosting للتصنيف (GBDT سكِتليرن)

from xgboost import XGBClassifier  # مصنّف XGBoost قوي على الجداول ويدعم عدم توازن بالفئة

from sklearn.linear_model import LogisticRegression  # مصنّف لوجستي خطّي سريع ويطلع احتمالات واضحة

from catboost import CatBoostClassifier  # ممتاز مع (Categoricals) مباشرةً

import lightgbm as lgb
from lightgbm import LGBMClassifier  # LightGBM للتصنيف سريع وفعّال على الداتا الجدولية

# 2) Regression Models
# from sklearn.linear_model import LinearRegression  # انحدار خطّي لمهام Regression (مش تصنيف)
# from lightgbm import LGBMRegressor                 # LightGBM للانحدار (Regression)

# 3) أدوات القياس
from sklearn.metrics import make_scorer  # بناء Scorer مخصّص لاستخدامه في Grid/CV

# from sklearn.metrics import mean_squared_error as MSE  # مقياس للـ Regression (مش للتصنيف)
# from sklearn.metrics import mean_absolute_error  # مقياس للـ Regression (مش للتصنيف)

# 4) ضبط الهايبر بارامترز (Optimization)
from hyperopt import hp, fmin, tpe  # أفضل قيم للسوبر-بارامترز (زي n_estimators, max_depth, learning_rate في XGBoost). بدل ما نجرّب يدوي
from bayes_opt import BayesianOptimization  # لضبط هايبر باراميترز نماذج مثل LightGBM/XGBoost/CatBoost بسرعة وكفاءة.

# 5) تقسيمات وتقييم بالنمط المتقاطع (CV)
from sklearn.model_selection import GridSearchCV, StratifiedKFold
# GridSearchCV: بحث شبكي منظّم على قيم محدّدة. >>>>> StratifiedKFold: تقسيم K-Fold يحافظ على نسبة الفئات مهم جدًا مع عدم التوازن

from sklearn.model_selection import KFold, cross_val_score  # KFold بدون حفظ نسب الفئات + حساب نتائج CV بسرعة
...
mnb = MultinomialNB()  # مصنّف بايز; يتوقّع مدخلات غير سالبة (counts/TF)، مش مقيّسة بقيم سالبة

dtc = DecisionTreeClassifier()  # شجرة قرار للتصنيف

lrc = LogisticRegression()
rfc = RandomForestClassifier()
abc = AdaBoostClassifier()
bc  = BaggingClassifier()
etc = ExtraTreesClassifier()  # أشجار شديدة العشوائية؛ سريعة غالبًا

gbdt = GradientBoostingClassifier()
xgb = XGBClassifier()
cat = CatBoostClassifier()
from lightgbm import LGBMClassifier
lgb_clf = LGBMClassifier()  # تبني النموذج على مراحل و بتعتمد على أشجار القرار بهدف التصنيف بسرعة وكفاءة عالية

# جمع الموديلات في قاموس عشان نشغّل، نقارن، وندير نتائج موديلات كتير
...
def train_classifier(clf, X_train, y_train, X_test, y_test):
    clf.fit(X_train, y_train)  # تدريب الموديل على بيانات التدريب
    y_pred = clf.predict(X_test) # توليد التوقعات

    accuracy = accuracy_score(y_test, y_pred)
    precision = precision_score(y_test, y_pred)
    recall = recall_score(y_test, y_pred)

    # حساب CM to get >>> TN and FP
    tn, fp, fn, tp = confusion_matrix(y_test, y_pred).ravel()  # ravel() >>> يفترض مصفوفة 2×2
    specificity = tn / (tn + fp)
    # نسبة السالب الحقيقي اللي النموذج اكتشفه صح من كل السوالب الفعلية.
...
# XGB >> Recall -  0.77 >> الانسب عشان :
# “الموديل يتوقع إن فلان هيتخلف بس هو أصلًا مش هيتخلف” = False Positive
# مستعد أضحّي بشوية عملاء كويسين (FP)
# ميزيدش عندي عدد اللي هيتخلفوا عن السداد
...
# احسب نسبة عدم التوازن: scale_pos_weight ≈ (#negative / #positive)
...
def xgb_clf_bo(max_depth, learning_rate, subsample, colsample_bytree,
    ...
    params = {
        ...
        "eval_metric": "logloss",        # القياس الداخلي للموديل؛ التقييم الخارجي بالـ recall عبر CV
        "tree_method": "hist",
        "n_jobs": -1,
        "random_state": 12,
    ...
    # CV خفيف للتجريب الأولي؛ تقدّر تزوده لاحقًا
...
pbounds = {
    ...
    "spw_mult": (0.6, 1.6)  # نسمح بالتحريك حوالين النسبة الأساسية
}
...
# أفضل نتيجة (Recall) وأفضل بارامترز:
# xgb_bo.max

xgb_bo.max

# Best hyperparameters — XGBoost (مكافئ لخطوة LightGBM في الصورة)
import numpy as np

# نسبة عدم التوازن الأساسية لتحويل spw_mult إلى scale_pos_weight
pos = int(np.sum(y_train))
neg = len(y_train) - pos
base_spw = neg / max(1, pos)
...
best_xgb_params  # يطبع القيم النهائية

# Feature Importance (Classification) — RandomForestClassifier
# الغرض = تفسير وترتيب الفيتشرز

rfc.fit(X_new, y_train)
feature_scores = pd.Series(rfc.feature_importances_, index=X_new.columns).sort_values(ascending=False)
feature_scores

# اعلي فيتشرز تأثير في النتايج
...
assert 'xgb_bo' in globals(), "لازم xgb_bo يكون موجود (خليّة الـ BO متشغّلة)."
assert 'y_train' in globals(), "لازم y_train يكون موجود."
...
import json
assert 'X_new' in globals(), "X_new لازم يكون موجود."
...
# ترتيب منطقي للفئات
groups = {
    "درجة ائتمانية":        ["A","B","C","D","E","F","G"],
    "وضع سكن":             ["MORTGAGE","RENT","OWN","OTHER"],
    "غرض القرض":           ["DEBTCONSOLIDATION","EDUCATION","HOMEIMPROVEMENT","MEDICAL","PERSONAL","VENTURE"],
    "شريحة/باند":          ["high","high-middle","middle","low-middle","low"],
    "فئة عمرية":           ["20-25","26-35","36-45","46-55","56-65"],
    "حجم/باند":            ["small","medium","large","very large"],
    "ثنائي":               ["N","Y"],
}
...
# أي أعمدة أخرى (عددية/مشتقة) تتجمع هنا تلقائيًا
others = [c for c in feat_cols if all(c not in cols for cols in groups.values())]
if others:
    grouped["خصائص رقمية/مشتقة"] = others
...
assert all(v in globals() for v in ['X_new_test','y_test','X_new', 'best_xgb_params']), "لازم X_new, X_new_test, y_test, and best_xgb_params be available."
...
# 2) اختار عيّنة
idx = 0  # ← غيّرها لتجربة عميل تاني
row_std = X_new_test.iloc[[idx]]
y_true = int(np.array(y_test)[idx])

# 3) تعريف المجموعات (لعرض الفئات كبشر)
groups = {
    "Grade":          ["A","B","C","D","E","F","G"],
...
# 4) نحاول نجيب القيم الخام (RAW) تلقائيًا:
raw_numeric_cols = [
...
def find_raw_df(index_label):
    ...
    # استبعد الداتا المقيّسة (زي X_new / X_new_test)
...
def inverse_with_scaler():
    # نحاول نلاقي StandardScaler متخزن
    scaler = None
...
ok    = "✅ Correct" if pred == y_true else "❌ Incorrect"
...
print("\n— Customer values (RAW view) —")
if raw_vals:
    for k in raw_numeric_cols:
        if k in raw_vals:
            print(f"  {k}: {raw_vals[k]:,.2f}")
else:
    print("  (القيم الخام غير متاحة تلقائيًا؛ المعروض أدناه قيَم مُقاسة z-score)")
    for k in raw_numeric_cols:
        if k in row_std.columns:
            print(f"  {k} (z): {float(row_std[k].iloc[0]):.3f}")

print(f"\n🔮 Model: {lbl}  (score={score:.4f})")
print(f"📌 Actual: {actual}  (y_test={y_true})")
print(f"🧪 Evaluation: {ok}")

# Binary flag >>> هل عنده تعثّر سابق مسجّل؟
# Band >>> شدة مخاطرة

# القيم السالبه هي القيم بعد ما اتعملها >> Standardization

# ليه بنعمل Standardization للقيم “الطبيعية” زي العمر/الدخل/مبلغ القرض؟
# مثلًا الدخل ممكن يبقى من 2,000 لـ 200,000، بينما الفايدة من 5 لـ 25 >>
# لو سبتهم خام، خوارزميات بتعتمد على المسافة أو الإسقاط هتخلي الميزة الكبيرة في المدى (الدخل) تهيمن على القرار

# ===== Mini GUI (self-healing) — trains if model not fitted =====
...
def _get_training_data():
    # نفضّل X_new / y_train لو موجودين (نفس اللي اتدرّبت عليه النوتبوك)
    X = _find_first(['X_new','X'])
    y = _find_first(['y_train','y'])
    if X is None or y is None:
        raise AssertionError("مش لاقي بيانات تدريب (عايز X_new/y_train أو X/y). شغّل خلايا التحضير/التدريب الأول.")
    # لو NumPy حوله DataFrame عشان الأعمدة
...
def _ensure_fitted_model():
    ...
    # لو وصلنا هنا يبقى لسه مفيش موديل fitted — جرّب ندرب XGB سريعًا
    X_tr, y_tr = _get_training_data()
    from xgboost import XGBClassifier

    # لو عندنا best_xgb_params استخدمها، غير كده defaults خفيفة
    params = {
    ...
    if 'best_xgb_params' in globals():
        ...
        # لو عندك عدم توازن كبير ومتوفر y:
...
def _feature_names_from_model_or_data(model):
    ...
    raise AssertionError("مش لاقي أسماء الأعمدة. لازم X_new/X_new_test أو model.feature_names_in_.")

def _find_scaler():
    # محاولة إيجاد StandardScaler (اختياري)
    for obj in globals().values():
        if hasattr(obj, "mean_") and hasattr(obj, "scale_") and hasattr(obj, "feature_names_in_"):
            return obj
    return None
...
groups = {k:v for k,v in groups.items() if v}  # شيل الفارغة

# -------- numeric inputs (RAW) ----------
numeric_raw = [
    ("person_age", "العُمر (سنة)", 30.0),
    ("person_income", "الدخل السنوي", 30000.0),
    ("person_emp_length", "مدة العمل (سنة)", 3.0),
    ("loan_amnt", "مبلغ القرض", 10000.0),
    ("loan_int_rate", "سعر الفائدة ٪", 12.0),
]
raw_numeric_cols = [
...
def build_feature_row():
    ...
    cred_hist_len = 5.0  # تقدير بسيط لو مفيش إدخال
    ...
    raw_fill = {
        ...
        "loan_int_rate": vals.get("loan_int_rate", 0.0)/100.0,  # لو عمودك مُخزّن كنسبة عشرية
...
def on_click(_):
    with out:
        ...
        try:
            ...
            print(f"🧠 Model: {model_name}   |   just_trained: {just_trained}   |   used_scaler: {used_scaler}")
